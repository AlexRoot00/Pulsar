...
//! Full key (BPF uses L4_ACL_PREFIX_LEN) = 44 bytes = 352.
//! IMPORTANT: the base case (any/any) gives 272, not 304 — exactly this key
//! actually works on the device (AclDrop grows, traffic is dropped).
...
impl Ipv4 {
    ...
    pub fn parse(s: &str) -> Result<Ipv4, String> {
        ...
        let prefix = if let Some(p) = prefix_str {
            let p_bits: u32 = p.parse().map_err(|_| format!("invalid prefix: '{p}' in '{s}'"))?;
            if p_bits > 32 {
...
impl Ipv6 {
    ...
    fn parse_addr(s: &str) -> Result<[u8; 16], String> {
        ...
        if expanded.len() != 8 {
            return Err(format!("incorrect number of hextets in '{s}': {}", expanded.len()));
        }
    ...
    /// Create an Ipv6 from raw address and prefix.
...
impl AclRule {
    ...
    pub fn build_key(&self) -> Vec<u8> {
        ...
        let sport_raw = if is_icmp { 0 } else {
            match self.sport {
                PortSpec::Exact(s) => s,
                PortSpec::Any => 0,
            }
        };
        let dport_raw = if is_icmp { 0 } else {
            match self.dport {
                PortSpec::Exact(d) => d,
                PortSpec::Any => 0,
            }
...
mod tests {
    ...
    fn test_ipv4_cidr_prefixes() {
        ...
        assert_eq!(rule.prefix_len(), 296, "IPv4 /24 dst prefix should be 296 bits");
        ...
        assert_eq!(rule.prefix_len(), 288, "IPv4 /16 dst prefix should be 288 bits");
        ...
        assert_eq!(rule.prefix_len(), 280, "IPv4 /8 dst prefix should be 280 bits");
        ...
        assert_eq!(rule.prefix_len(), 272, "IPv4 /0 dst prefix should be 272 bits");
    }
    ...
    fn test_ipv6_cidr_prefixes() {
        ...
        assert_eq!(rule.prefix_len(), 304, "IPv6 /128 dst prefix should be 304 bits");
        ...
        assert_eq!(rule.prefix_len(), 272, "IPv6 /64 dst prefix should be 272 bits");
        ...
        assert_eq!(rule.prefix_len(), 272, "IPv6 /96 dst prefix should be 272 bits");
        ...
        assert_eq!(rule.prefix_len(), 288, "IPv6 /112 dst prefix should be 288 bits");
    }
}