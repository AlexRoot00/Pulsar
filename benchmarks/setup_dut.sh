#!/usr/bin/env bash
#
# setup_dut.sh — setup VLAN/QinQ interfaces on DUT.
# Creates sub-interfaces with IP .2 (GEN listens on .1).
# Used for S4 (VLAN ACL QinQ) scenarios from spec §6.
#
# Requirements: root, interface IF (default enp6s18).
# XDP is attached to physical IF — XDP sees raw frames with 802.1Q tags.
# Sub-interfaces needed for IP stack (L3 connectivity check), XDP
# parses VLAN tags in raw frame before kernel extracts them.
#
set -euo pipefail

IF="${IF:-enp6s18}"

echo "=== DUT VLAN/QinQ setup (iface=$IF) ==="
...
ip addr add 192.168.30.2/24 dev "${IF}.300"
...
ip addr add 192.168.140.2/24 dev "${IF}.140"
...
ip addr add 192.168.100.2/24 dev "${IF}.200.100"
...
ip addr add 192.168.101.2/24 dev "${IF}.200.101"
...
ip addr add 192.168.130.2/24 dev "${IF}.130.100"

echo
echo "========================================"
echo " DUT configuration complete"
echo "========================================"
echo
ip -br addr show "$IF"
